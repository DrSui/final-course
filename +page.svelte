<script>
  ...
  async function signUp() {
    const { data: exists, error: checkError } = await supaClient
      .from('users')
      .select('*')
      .eq('email', sign_email);

    if (checkError) return alert(`Error: ${checkError.message}`);
    if (exists.length > 0) return alert('Email already registered.');

    const { data, error } = await supaClient
      .from('users')
      .insert([{ email: sign_email, password: sign_passwrd }])
      .select()
      .single();
  ...
  function signOut() {
    ...
    userId = null;
    apiKeys = [];
    sign_email = sign_passwrd = log_emial = log_passwrd = '';
  }
</script>
  ...
  {#if !logged}
    <div class="auth-container">
      <div class="auth-section">
        <h2>Sign Up</h2>
        <input type="email" bind:value={sign_email} placeholder="Email" />
        <input type="password" bind:value={sign_passwrd} placeholder="Password" />
        <button on:click={signUp}>Sign Up</button>
      </div>

      <div class="auth-section">
        <h2>Login</h2>
        <input type="email" bind:value={log_emial} placeholder="Email" />
        <input type="password" bind:value={log_passwrd} placeholder="Password" />
